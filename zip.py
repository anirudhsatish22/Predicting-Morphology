...
from reformatdata import baseline
...
for root, _, files in os.walk("trainHigh", topdown=False):
    for name in files:
        if not name.startswith("."):
            ...
            language = "reformatted-baseline/" + name[:-11]
            # print(language)
            ...
            baseline(inputFp, outputFp, True)
...
for root, _, files in os.walk("testData", topdown=False):
    for name in files:
        if not name.startswith("."):
            full_input_path = os.path.join(root, name)
            language = "reformatted-baseline/" + name[:-5]
            # print(language)
            full_outputPath = os.path.join(language, name)
            # print(splitList)
            ...
            inputFp = open(full_input_path, "r")
            outputFp = open(full_outputPath, "w")
            baseline(inputFp, outputFp, False)
